...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    if (!CheckProofOfWork(block.GetPoWHash(Params().SwitchLyra2REv2Block(nHeight)), block.nBits, consensusParams))
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetPoWHash(Params().SwitchLyra2REv2Block(nHeight)), block.nBits, consensusParams))
